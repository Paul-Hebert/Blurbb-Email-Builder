<?php 
    $title = 'Resources';
    $base_url = '../';    
	include('../includes/page_sections/header.php'); 
?>
		<div class="main_content">
			<div class="col-lg-6 col-lg-offset-3">      
				<h1>Resources</h1>
				<section>
					<h2>Email Coding</h2>
					<ul>
						<li><a href="http://www.fonts.com/content/learning/fyti/using-type-tools/fonts-on-the-web">Web Safe Fonts</a></li>
						<li><a href="https://www.campaignmonitor.com/" target="_BLANK">Campaign Monitor</a></li>				
						<li><a href="https://www.campaignmonitor.com/css/" target="_BLANK">Campaign Monitor's Ultimate Guide to CSS</a></li>
						<li><a href="http://templates.mailchimp.com/resources/inline-css/" target="_BLANK">Mailchimp's CSS inliner</a></li>
					</ul>
				</section>
				<section>
					<h2>Email Testing</h2>
					<ul>
						<li><a href="https://litmus.com/" target="_BLANK">Litmus</a></li>
					</ul>
				</section>
				<section>
					<h2>Email Delivery</h2>
					<ul>			
						<li><a href="http://mailchimp.com/" target="_BLANK">Mailchimp</a></li>
					</ul>
				</section>
			</div>
		</div>
	    <?php
	      include($base_url . 'includes/page_sections/scripts.php');
	    ?>
	</body>
</html>